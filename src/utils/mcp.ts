...
export async function toolsGuidanceBlock(names: string[] | undefined, inChat = false): Promise<string> {
  ...
  return `<tools_you_carry>\nThese tool servers are attached to this session. Do the work through their tools, not terminal workarounds (no python, pip or app binaries from the shell to reach what a tool already reaches) - the tools act where the person is looking.\nThe folder this session runs in is empty scratch space: nothing the person owns is in it. Notes, scenes, pages and files that belong to a tool exist only inside that tool - reach them with the tool's own read and search calls, never with read_file, list_dir or grep on the folder. Address them exactly as the tool lists them: list first, then use the listed name or relative path as it is, spaces and all - never put the session folder or a home folder in front of it, and never percent-encode it.\n${lines.join("\n")}${voice}</tools_you_carry>\n\n`;
}
