...
import { helperOfferEligible } from '../utils/homeOffers';
...
export default component$(() => {
  ...
  useVisibleTask$(async ({ cleanup }) => {
    ...
    try {
      const st = await modelManager.downloadStatus(UTILITY_MODEL.filename);
      if (st.downloading) {
    ...
    const unp = await listen<DownloadProgress>('model-download-progress', (e) => {
      if (e.payload.filename === UTILITY_MODEL.filename) {
        downloading.value = true;
  ...
  const doDownload = $(async () => {
    ...
    percent.value = 0;
    try {
      await modelManager.downloadModel(UTILITY_MODEL.downloadUrl, UTILITY_MODEL.filename, (p) => {
