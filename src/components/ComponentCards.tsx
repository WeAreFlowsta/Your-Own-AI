/**
 * The component cards - one per on-demand piece (capability models, the
 * Build agent). Used twice: Add-ons > Components (get: download or remove)
...
import { EMBEDDING_MODEL, modelFamilies, type CapabilityModel } from "../data/recommended-models";
...
export interface CardProps {
  ...
  stopCommand?: string;
  /** Settings > Storage: show only when installed, and only Remove. */
  manageOnly?: boolean;
}
...
export const ComponentCard = component$<CardProps>((props) => {
  ...
  if (props.manageOnly && !checking.value && !downloaded.value && !downloading.value) return null;
...
export const BuildComponentCard = component$<{ manageOnly?: boolean }>((props) => {
  ...
  if (props.manageOnly && !installed.value && !downloading.value) return null;
  return (
    <div class="flex items-start gap-4 p-4 rounded-xl bg-[var(--bg-main)] border border-[var(--border-subtle)]">
      <div class="w-10 h-10 rounded-lg bg-[var(--bg-dropdown)] flex items-center justify-center flex-shrink-0">
